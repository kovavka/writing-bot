...
export const texts = {
  ...
  setDeadline: `Когда ты хочешь закончить? Введи дату в формате ${DATE_FORMAT}`,
  setGoal: `Сколько слов ты хочешь написать за это время?`,
  projectCreated: (
    finalWords: number,
    daysLeft: number,
    dayGoal: number
  ): string => `WriteUp! Время писать! Через ${daysLeft} ${getWordForm(daysLeft, forms.days)} в твоём гримуаре должно быть ${finalWords} ${getWordForm(finalWords, forms.words)}.
