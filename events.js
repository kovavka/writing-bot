...
const marathon = {
  ...
  progress: [1000, 1200, 2000, 2000, 3500, 6000, 60001],
}
...
Например \`/join 2000\`, чтобы присединиться с начальными 2000 слов или просто \`/join\`, чтобы начать с чистого листа.`,
            ...
            .catch(error => {
              console.error(
                `Failed to send message to chat ${ADMIN_ID}: ${error.message}`
              )
            })
        }, 1000)
        ...
        setTimeout(() => {
          ...
          bot.telegram
            .sendMessage(
              ADMIN_ID,
              `Спринт закончился. У вас будет 5 минут, чтобы ввести свой результат.
...
bot.command('join', ctx => {
  ...
  ctx.reply(
    `Вы присоединились к спринту. Начальное количество слов: ${wordCount}`
  )
  // add to db
})
...
bot.on('callback_query', ctx => {
  ...
  if (callbackData.startsWith('run_event')) {
    ...
    ctx.reply(
      `Вы больше не будете получать уведомления о спринтах в текущем событии.
